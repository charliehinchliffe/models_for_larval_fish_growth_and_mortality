---
title: "Comparing the effect of sample size on model efficacy, when growth rates are taken from literature"
...
The .rmd demonstartes the effect of sample size on classic catch-curve methods, our continous catch-curve, and our size-distribution model with known G. To age the data a simple linear model is applied to size_sampled using a specificed G from the literature (0.21)
...
# Select sample sizes
hatches_per_day <- c(10,10,10,10,10,10,10,10,10,10,
...
saveRDS(output_data, "output/literature_growth_R.rds")
```

# Read in output
```{r}
output_data <- readRDS("output/literature_growth_R.rds")
```
...
p3 <- ggplot(R5, aes(x = model, y = z, fill = model, group = model)) +
...
lit_g_ss_fac_plot <- ggplot(data, aes(x = model, y = z, fill = model, group = model)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), trim = FALSE) +
  geom_hline(yintercept=0.25, color = "red", size=0.5, alpha = 0.5) +
  theme_bw() +
  theme(axis.title.x=element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank())+
...
figure_4 <- plot_grid(lit_g_ss_fac_plot, lit_g_z_var_fac_plot, lit_g_s0_var_fac_plot, lit_g_s0_var_with_trim_fac_plot, lit_g_difference_g_fac_plot, align = c("hv"),
  axis = c("none"),
  nrow = 5, ncol = 1, rel_widths = 1, rel_heights = 1,
  labels = c('A', 'B', 'C', 'D', 'E'))
...
#Satn size
SA5 <- output_data %>%
 select(hatches_per_day, stan_size) %>%
  filter(hatches_per_day == 5) %>%
  select(stan_size) 
...
mean(NaRV.omit(SA200$stan_size))
sd(NaRV.omit(SA200$stan_size))
...
normal_lit_ss_var_fac_plot <- ggplot(data, aes(x = model, y = z, fill = model, group = model)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), trim = FALSE) +
  geom_hline(yintercept=0, color = "red", size=0.5, alpha = 0.5) +
  theme_bw() +
  theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank())+
...
# combine all lit g plots 
```{r, eval = False}
library(cowplot)

figure_4 <- plot_grid(normal_lit_ss_var_fac_plot, normal_lit_g_z_var_fac_plot, normal_lit_g_s0_var_fac_plot, normal_lit_g_s0_var_with_trim_fac_plot, normalised_lit_g_difference_g_fac_plot, align = c("hv"),
  axis = c("none"),
  nrow = 5, ncol = 1, rel_widths = 1, rel_heights = 1,
  labels = c('A', 'B', 'C', 'D', 'E'))
