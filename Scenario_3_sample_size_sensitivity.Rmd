---
title: "Comparing the effect of sample size on model efficacy, when fish are aged with linear model"
...
The .rmd demonstartes the effect of sample size on classic catch-curve methods, and our size-distribution model. To 'age' the data we use a linear model on a subset of the population simulated using simulate_catch_data(). 
...
data2 <- create_bins(data2)

data <- age_data_with_linear_model(data, n_samples_each = 5)
data2 <- age_data_with_linear_model(data2, n_samples_each = 5)
```
...
##Stan size dist growthmodel data
...
saveRDS(output_data, "output/linear_model_ages_R.rds")
```

# Read in output
```{r}
output_data <- readRDS("output/linear_model_ages_R.rds")
```
...
```{r}
fac_plot <- ggplot(output_data_long, aes(x = model, y = z, fill = model, group = model)) +
  ...
  facet_wrap(~hatches_per_day, nrow = 1, ncol = 7)
...
linear_model_ss_fac_plot <- ggplot(data, aes(x = model, y = z, fill = model, group = model)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), trim = FALSE) +
  geom_hline(yintercept=0.25, color = "red", size=0.5, alpha = 0.5) +
  theme_bw() +
  theme(axis.title.x=element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank())+
...
linear_model_ss_fac_plot
```

combine linear model plots 
```{r, eval = False}
library(cowplot)

figure_5 <- plot_grid(linear_model_ss_fac_plot, linear_model_z_var_fac_plot, linear_model_s0_var_fac_plot, linear_model_s0_var_with_trim_fac_plot, linear_model_g_var_fac_plot, linear_model_N_for_growth_fac_plot, align = c("hv"),
  axis = c("none"),
  nrow = 6, ncol = 1, rel_widths = 1, rel_heights = 1,
...
#Satn size
SA5 <- output_data %>%
 select(hatches_per_day, stan_size_growth) %>%
  filter(hatches_per_day == 5) %>%
...
mean(NaRV.omit(SA200$stan_size_growth))
sd(NaRV.omit(SA200$stan_size_growth))
...
normal_linear_mod_ss_var_fac_plot <- ggplot(data, aes(x = model, y = z, fill = model, group = model)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), trim = FALSE) +
...
normal_linear_mod_ss_var_fac_plot
```
...
figure_5 <- plot_grid(normal_linear_mod_ss_var_fac_plot, normal_linear_mod_z_var_fac_plot, normal_linear_mod_s0_var_fac_plot, normal_linear_mod_s0_var_with_trim_fac_plot, normal_linear_mod_g_var_fac_plot, normal_linear_model_N_for_growth_fac_plot, align = c("hv"),
  axis = c("none"),
  nrow = 6, ncol = 1, rel_widths = 1, rel_heights = 1,
  labels = c('A', 'B','C','D','E','F' ))
